...
for time = 2:nTimeSteps
    ...
    drainage = -k(1) * (2 * (surfaceData(time) - h0(1)) / dz + 1);
    surfaceData(time + 1) = surfaceData(time) + (rainfall - drainage) * dt;
    if surfaceData(time + 1) < 0
        surfaceData(time + 1) = 0;
        drainage = rainfall + surfaceData(time) / dt;
    end
